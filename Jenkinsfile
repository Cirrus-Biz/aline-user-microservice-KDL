            ...
            steps{
                script{
                    withSonarQubeEnv(installationName: "sonarqube") {
                        bat "mvn clean test verify package sonar:sonar -Dsonar.projectKey=users-microservice-kdl"
                        }
                }
        ...
        stage("Build Image w/docker"){
            steps{
                script{
                    COMMIT = "${GIT_COMMIT}"
                    SLICE = COMMIT[1..7]
                    bat "docker build -t $env.AWS_ECR_REGISTRY/users-microservice-kdl:${SLICE}.${BUILD_NUMBER}.${DATE} ."
                }
