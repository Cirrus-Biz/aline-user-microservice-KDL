        ...
        stage("Build Image w/docker"){
            steps{
                script{
                    COMMIT = "${GIT_COMMIT}"
                    SLICE = COMMIT[1..7]
                    sh "sudo docker build -t $env.AWS_ECR_REGISTRY_WEST/users-microservice-kdl:${SLICE}.${BUILD_NUMBER}.${DATE} ."
                    sh "sudo docker build -t $env.AWS_ECR_REGISTRY_WEST/users-microservice-kdl:${SLICE}.${BUILD_NUMBER}.${DATE} ."
                }
            ...
            steps{
                script{
                   ...
                   docker.withRegistry("https://$env.AWS_ECR_REGISTRY_WEST", "ecr:$env.AWS_REGION_WEST:AWS-KDL"){
                       docker.image("$env.AWS_ECR_REGISTRY_WEST/users-microservice-kdl:${SLICE}.${BUILD_NUMBER}.${DATE}").push()
                       docker.image("$env.AWS_ECR_REGISTRY_WEST/users-microservice-kdl:${SLICE}.${BUILD_NUMBER}.${DATE}").push("latest")
                   }
                }
            }
        }
